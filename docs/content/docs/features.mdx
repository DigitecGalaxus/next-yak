...
<Callout type="info">
  As of Next.js ≥15.0.0, dynamic functions like `cookies()` and `headers()` are
  async. Next-yak supports async `getYakThemeContext()` functions, allowing
  you to await these server-side APIs seamlessly.
</Callout>
...
const Component = (props) => {
  return (
    <div css={atoms("bg-red-500", props.primary && "bg-blue-500")}>
      Hello there!
    </div>
  );
};
