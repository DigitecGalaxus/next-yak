...
For styles that don't belong to a single component like CSS resets, `@font-face` declarations, `:root` variables or view transitions, you can use the `globalCss` API. The styles are extracted at build time like all other yak styles and applied as soon as the file declaring them is imported, typically from your root layout or page entry.
...
Runtime values (`${(props) => ...}`) are not supported inside `globalCss` as a global rule has no element to attach them to. Declare a CSS custom property instead and toggle it via an attribute or class on the root element:
...
<Callout type="info">
  `globalCss` must be declared at module scope — using it inside a component or function is a
...
<Callout type="warn">
  When using Next.js with webpack, class selectors for markup you don't control (e.g. a third-party
  widget's `.maps` class) need to be wrapped in `:global()` to escape CSS Modules scoping:
  `:global(.maps) {/* ... */}`. In native CSS mode (Vite, Turbopack, Rsbuild) the same selector must be
  written without the wrapper — plain `.maps {/* ... */}` just works there.
</Callout>
