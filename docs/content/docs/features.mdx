...
<Callout type="info">
  ...
  Next.js with webpack they are hashed by default, so enable native CSS transpilation mode
  (`transpilationMode: 'Css'`) to keep them unchanged. The `:global(.maps)` wrapper still works as a
  deprecated fallback, but like every `:global()` selector it is on its way out.
