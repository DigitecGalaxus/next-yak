...
For styles that don't belong to a single component like CSS resets, `@font-face` declarations, `:root` variables or view transitions, you can use the `globalStyles` API. The styles are extracted at build time like all other yak styles and applied as soon as the file declaring them is imported, typically from your root layout or page entry.
...
Runtime values (`${(props) => ...}`) are not supported inside `globalStyles` as a global rule has no element to attach them to. Declare a CSS custom property instead and toggle it via an attribute or class on the root element:
...
<Callout type="info">
  `globalStyles` must be declared at module scope — using it inside a component or function is a
...
<Callout type="info">
  Class selectors for markup you don't control (e.g. a third-party widget's `.maps` class) must never
  be hashed. On native CSS pipelines (Vite, Turbopack, rsbuild) they stay intact automatically. On
  Next.js with webpack, css-loader scopes them. Enable native CSS transpilation mode
  (`transpilationMode: 'Css'`) so they pass through unchanged. The `:global(.maps)` wrapper still works
  as a deprecated fallback, but like every `:global()` selector it is on its way out.
