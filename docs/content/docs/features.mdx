...
<Callout type="info">
  As of Next.js ≥15.0.0, dynamic functions like `cookies()` and `headers()` are async. Next-yak now supports async `getYakThemeContext()` functions, allowing you to await these server-side APIs seamlessly.
</Callout>
