...
Runtime values (`${(props) => ...}`) are not supported inside `globalCss` — a global rule has no element to attach them to. Declare a CSS custom property instead and toggle it via an attribute or class on the root element:
