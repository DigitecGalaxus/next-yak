...
While next-yak is built primarily for Next.js, you can also use it in a plain React + Vite setup via the `viteYak` plugin.
...
Add the `viteYak` plugin to your `vite.config.ts` (or `.js`). It works together with `@vitejs/plugin-react-swc`:
