...
## Other linters (oxlint & rslint)

`eslint-plugin-yak` fully supports oxlint and rslint. A good starting configuration for each:

<Tabs items={['oxlint', 'rslint']}>
<Tab value="oxlint">
```json title=".oxlintrc.json"
{
  ...
  "rules": {
    ...
    "yak/precompute-style-prop-values": "warn"
  }
}
```
</Tab>
...
### css-global-deprecated

Warns about usage of `:global()` selectors, which are deprecated and will be removed in the next major version in favor of native CSS transpilation.
...
The `:global()` selector was originally needed to opt out of CSS Modules scoping. With the new native CSS transpilation mode, styles are no longer scoped through CSS Modules, making `:global()` unnecessary.

To migrate, enable native CSS transpilation in your `next.config.js`:

```js
experiments: { transpilationMode: 'Css' }
```

See the [migration guide](/docs/migration-to-native-css) for more details.
...
const App = () => <Sticker $tilt={Math.random() * 10} />; // [!code --]
const App = () => { // [!code ++]
  const tilt = Math.random() * 10; // [!code ++]
  return <Sticker $tilt={tilt} />; // [!code ++]
}; // [!code ++]
```

#### Reason why

next-yak inlines a styled component into its usage at build time and copies the value of a prop into every style condition that reads it. Each condition above rolls its own number, so the sticker can get the shadow without the rotation, even though a tilt above 8 is always above 5. That is a combination the component could never render on its own.

A prop that reaches the DOM is kept on the element as well, so `<Button disabled={Math.random() > 0.5} />` rolls once for the attribute and once for the condition, and the button can end up disabled while it is styled as enabled.

Passing a precomputed variable fixes both: an identifier is only read, never evaluated again, so the component is still inlined. Pure values such as `Math.max(4, s)` are reported too, because precomputing them keeps the generated code smaller.

See the [rule documentation](https://github.com/DigitecGalaxus/next-yak/blob/main/packages/eslint-plugin-yak/docs/rules/precompute-style-prop-values.md) for the exact conditions under which the rule reports.
