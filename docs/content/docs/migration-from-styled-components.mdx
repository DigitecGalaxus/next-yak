...
- Mixins
- Global styles (`createGlobalStyle` becomes [`globalStyles`](/docs/features#global-styles))
...
`createGlobalStyle` returns a component that you render. In next-yak global styles are a statement.
[`globalStyles`](/docs/features#global-styles) applies as soon as the file declaring it is imported, so
remove the JSX usage and import the file at the position where you used to render `<GlobalStyle />`. For
app-wide resets that is your root layout, but if it only rendered on a certain page, import the file from
that page instead:

```tsx title="globals.tsx"
import { createGlobalStyle } from "styled-components"; // [!code --]
import { globalStyles } from "next-yak"; // [!code ++]
...
[Global styles](/docs/features#global-styles) for the patterns.
