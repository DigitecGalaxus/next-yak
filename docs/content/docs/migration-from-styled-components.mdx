...
One of the core goals of `next-yak` is to provide a smooth migration path from `styled-components` to `next-yak`.
This guide walks you through the migration, including common scenarios and edge cases.

## Eslint plugin

To make the migration easier, we provide an eslint plugin which will help you to identify potential issues during the migration and afterwards.
...
- Mixins
- Global styles (`createGlobalStyle` becomes [`globalStyle`](/docs/features#globalstyle))
- If some features are missing, please let us know and open a feature request on [GitHub](https://github.com/DigitecGalaxus/next-yak/issues/new/choose)
...
<Callout>
  **Important note on semicolons**: You need to add a semicolon after the mixin to separate it from
  the rest of the styles. Otherwise the parser can not differentiate between a selector, a constant
  or a mixin.
</Callout>
...
You can't nest arbitrary styles inside property values, but you can nest property values inside css tags.
...
styled-components `createGlobalStyle` returns a component that you render. In next-yak global styles are a statement and [`globalStyle`](/docs/features#globalstyle) applies as soon as the file with the global styles are imported.
So to migrate your global styles, you need to remove the Global Style JSX (e.g. `<GlobalStyle />`) usage and import the file at the position where you used to render `<GlobalStyle />`. For
app-wide resets that is your root layout, but if it only rendered on a certain page, import the file from that page instead:

```tsx title="globals.tsx"
import { createGlobalStyle } from "styled-components"; // [!code --]
import { globalStyle } from "next-yak"; // [!code ++]
...
import { GlobalStyle } from "./globals"; // [!code --]
import "./globals"; // [!code ++]
...
Theme props like `${(props) => props.theme.color}` inside global styles become CSS custom properties
toggled via an attribute or class on the root element. And a conditionally rendered `<GlobalStyle />`
becomes a CSS condition (`@media`, `@supports`, `:root[data-theme]`) — see
[Global styles](/docs/features#globalstyle) for the patterns.

### Reference external styles

If you reference external styles (styles that aren't component styles) in your styled components, you have two options:

**Option 1 (Recommended): Enable native CSS transpilation mode**

Enable native CSS mode in your next.config.js, then external selectors work without any wrapper:
...
**Option 2 (Deprecated): Use `:global()` selector**

<Callout type="warn">
  **Deprecation Notice**: The `:global()` selector syntax is deprecated and will be removed in the
  next major version. Please migrate to native CSS transpilation mode. For more information read
  [Migration to native CSS](/docs/migration-to-native-css).
</Callout>

If you're using the default CSS Modules mode, you need to wrap external selectors with `:global()`:
...
This is needed because next-yak transforms your styles to CSS modules by default. More can be found in the [docs of css-modules](https://github.com/css-modules/css-modules/blob/master/docs/composition.md#exceptions)
...
But you also need to tell TypeScript that native JSX elements can have a css prop. To do that, add the following to your `tsconfig.json`.
...
<Callout>
  **Important note on `.yak.tsx` files**: As yak files are evaluated on every code change, they will
  slow down your dev experience and your production build times. They should only be used as a last
  resort.
</Callout>
...
A general rule of thumb is that next-yak doesn't run any JavaScript code to figure out any interpolated values except the code resides in a `.yak.ts`(or `.yak.js`) file or is imported from a yak file.

Everything except calculations and string concatenations, or more generally, values that don't need a runtime in order to have a value, should
...
If you can't figure it out, please open an issue on [GitHub](https://github.com/DigitecGalaxus/next-yak/issues/new/choose)

### I think I found a bug

Please open an issue on [GitHub](https://github.com/DigitecGalaxus/next-yak/issues/new/choose)
