...
<Callout type="warn">
**Deprecation Notice**: The `:global()` selector syntax is deprecated and will be removed in the next major version. Please migrate to native CSS transpilation mode. For more information read [Migration to native CSS](/docs/migration-to-native-css).
</Callout>
...
<Callout type="warn">
**Turbopack and Vite limitation**: Yak files with imports are not yet supported in Turbopack or Vite. Yak files must be self-contained in these environments.
</Callout>
...
const yakConfig: YakConfigOptions = {
  experiments: {
    // Enable debug mode.
    // - Use 'true' for all files
    // - Use a regex string to filter by path (e.g., "Logo", "page")
    // - Use ".css$" to filter raw CSS or ".css-resolved$" for resolved CSS
    debug: 'Logo'
  }
}
