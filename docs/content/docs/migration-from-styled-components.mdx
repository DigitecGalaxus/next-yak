...
Dynamic mixins can also be exported and used in other files - just like in styled-components:

```tsx title="mixin.ts"
import { css } from "styled-components"; // [!code --]
import { css } from "next-yak"; // [!code ++]
...
Two things are worth knowing about how this compiles:

- **Importing a mixin keeps its module in the bundle.** The component holds a
  runtime reference to the imported mixin, so the mixin's file (including e.g.
  `keyframes` it defines) is never tree-shaken away while the component is used.
- **CSS variables keep the name of the file that defines the mixin.** A dynamic
  value like `padding: ${(props) => props.$pad}px` uses the same css variable
  name for every component that uses the mixin (unlike same-file mixins, which
  get a per-component name). This is only visible in devtools - the values are
  still set per element.

Dynamic mixins can only be interpolated as statements (`${mixin};`) - not
inside a property value (`color: ${mixin};`) or as a selector.
