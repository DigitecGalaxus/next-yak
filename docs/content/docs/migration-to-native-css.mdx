...
`html` and `body` are document-level, not scoped to a component, so use the [`globalStyle`](/docs/features#globalstyle) API. It targets element selectors directly on every bundler, no `:global()` needed:
