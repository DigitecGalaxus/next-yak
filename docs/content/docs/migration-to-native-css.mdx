...
<Callout type="warn">
  **Deprecation Notice**: The `:global()` selector syntax is deprecated. Please migrate to native
  CSS transpilation mode to ensure compatibility with future versions.
</Callout>
...
<Callout type="warn">
  This is only a temporary solution. Please plan to complete your migration before the next major
  version.
</Callout>
...
`html` and `body` are not scoped to a component, so wrapping them in a `styled` component was always a workaround. Reach for the [`globalStyle`](/docs/features#globalstyle) API instead, which targets element selectors directly on every bundler without `:global()`:
