...
// @qwik.dev/* and the aliasable context entry stay external
const runtimeExternals = [/^@qwik\.dev\//, /^@yak\/qwik\/context$/];
...
export default defineConfig([
  ...
  // it via `declare module "@yak/qwik"`. The JS build above keeps the
  // context external so there is exactly one runtime context instance.
  {
    entry: ["runtime/index.ts"],
    format: ["esm"],
    dts: { emitDtsOnly: true },
    deps: { neverBundle: [/^@qwik\.dev\//] },
    target: "es2022",
    outDir: "dist",
    outExtensions,
  },
  ...
  // theme context (separate entry so the vite plugin can alias baseContext)
  {
    ...
    sourcemap: true,
    dts: true,
    deps: { neverBundle: [/^@qwik\.dev\//, /^@yak\/qwik($|\/)/] },
    target: "es2022",
    outDir: "dist/context",
    outExtensions,
    outputOptions: stripJsdoc,
