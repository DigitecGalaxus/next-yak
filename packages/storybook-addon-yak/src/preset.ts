...
export async function viteFinal(config: any, options: StorybookOptions) {
  ...
  try {
    const vitePluginModule = await import("next-yak/loaders/vite-plugin");
    const yakPlugin = await vitePluginModule.viteYak({
...
/**
 * SWC configuration for webpack
 */
export function swc(swcConfig: any, options: StorybookOptions) {
  // Note: This is called synchronously, but the plugin path should be resolved
  // by the time webpack actually uses it (after webpackFinal runs first)
  return swcConfig;
}
