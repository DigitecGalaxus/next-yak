...
import { YakThemeContext, YakThemeProvider } from "../context/index.ts";
...
it("should support subtree overrides through YakThemeContext", () => {
  const Box = styled.div("box", {
    style: { "--color": (props: any) => props.theme().color },
  });
  const [color, setColor] = createSignal("red");
  const container = renderInto(() => {
    const override = createMemo(() => ({ color: color() }));
    return (
      <YakThemeProvider theme={{ color: "outer" }}>
        <YakThemeContext value={override}>
          <Box />
        </YakThemeContext>
      </YakThemeProvider>
    );
