...
- Changing html tags with the `as` prop ([alternative solution](#property-as-does-not-exist))
- `.withConfig`
...
`next-yak` does not support the `as` prop. Instead, it offers a type-safe alternative that lets you render dynamic HTML tags safely. You can create a reusable component that only allows a specific set of HTML tags. This prevents rendering invalid elements and improves type safety:
...
You can style this component just like any other component: 
```tsx
const Pink = styled(HtmlTag)`
  color: pink;
`;
```
