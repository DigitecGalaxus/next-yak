...
Starting with Next.js 15.0.0, dynamic functions at request time have been changed to be asynchronous (read this blog post about it)[https://nextjs.org/blog/next-15#async-request-apis-breaking-change]. This creates a limitation when using server-side dynamic values in `getYakThemeContext()`:
