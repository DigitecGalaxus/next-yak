...
export function getYakThemeContext() {
    return {
        brandName: process.env.NEXT_PUBLIC_BRAND_NAME || 'myDefaultBrand',
        featureFlags: {
...
We've created an [RFC to address this issue](https://github.com/DigitecGalaxus/next-yak/issues/403) and to provide a possible solution for it.
