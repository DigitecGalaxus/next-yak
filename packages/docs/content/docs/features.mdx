...
<Callout type="warn">
  As of Next.js ≥15.0.0, the `cookies()` function is async, which affects how you can use it within `getYakThemeContext`. See the note below for details.
</Callout>
...
export function getYakThemeContext() {
    return {
        brandColor: process.env.NEXT_PUBLIC_BRAND_COLOR || '#009688',
        featureFlags: {
...
Starting with Next.js 15.0.0, dynamic functions at request time have been changed to be asynchronous <a href="https://nextjs.org/blog/next-15#async-request-apis-breaking-change">read this blog post about it</a>. This creates a limitation when using server-side dynamic values in `getYakThemeContext()`:
...
- Use client-side state - Handle dynamic theming with React state and useEffect in client components
- Use static design tokens - For values that don't change at runtime (recommended for performance)

If you need server-side cookie-based theming, consider implementing it at the component level rather than in the global theme context.
