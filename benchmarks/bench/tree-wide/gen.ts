...
// Same Box+View shape as Tree, but breadth=750 / depth=1 (~1501 Boxes,
// React render depth 3). Cost is dominated by sibling array rendering
// rather than recursion. Pair with TreeDeep for the opposite shape.
const breadth = 750;
const depth = 1;
const wrap = 0;
...
export const TreeWide${isYak ? "Yak" : "Styled"}: FunctionComponent = () => (
  <Tree breadth={${breadth}} depth={${depth}} id={0} wrap={${wrap}} />
);
`;

  writeBenchmarkSource("TreeWide", lib, fileContent);
}

writeBenchmarkSource(
  "TreeWide",
  "vanilla",
  vanillaTreeSource("TreeWideVanilla", breadth, depth, wrap),
);
