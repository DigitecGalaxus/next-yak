...
test(
  "globalStyle applies a plain element selector to body",
  withTestEnv("global-style", async (testEnv, page) => {
...
test(
  "globalStyle declares a custom property on :root consumed by a component",
  withTestEnv("global-style", async (testEnv, page) => {
    await page.goto(testEnv.url);
...
test(
  "globalStyle can reference a keyframes animation",
  withTestEnv("global-style", async (testEnv, page) => {
    await page.goto(testEnv.url);
...
test(
  "globalStyle can target a component selector inside :has()",
  withTestEnv("global-style", async (testEnv, page) => {
    await page.goto(testEnv.url);
