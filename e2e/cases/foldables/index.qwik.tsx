...
export default component$(() => {
  ...
  return (
    <>
      ...
      <Toggle data-testid="toggle-state" $active={active.value}>
        state driven
      </Toggle>
      <button data-testid="activate" onClick$={() => (active.value = true)}>
        activate
      </button>
      ...
      {/* a static merge with a backslash escape in the user class - the
          folded class attribute must read the same as the runtime twin below */}
      <Card data-testid="escape" class="mark before:content-['\d7']">
        escape merge
      </Card>
      <Card data-testid="escape-runtime" {...{ class: "mark before:content-['\\d7']" }}>
        runtime twin
      </Card>
      {/* an emoji class must reach the DOM byte-exact, matching its twin */}
      <Card data-testid="emoji" class="🔥 mark">
        emoji merge
      </Card>
      <Card data-testid="emoji-runtime" {...{ class: "🔥 mark" }}>
        runtime twin
      </Card>
    </>
  );
