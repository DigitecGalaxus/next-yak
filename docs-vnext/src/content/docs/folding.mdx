...
```tsx title="output"
const Card = ⟦1:__yak.__yak_div("yCard")⟧;

const Page = () => ⟦2:<div className="yCard">hi</div>⟧;
```
...
```tsx title="input"
⟦1:<Card />⟧
<Card ⟦2:className="user"⟧ />
<Card ⟦3:className={on && "on"}⟧ />
```
...
```tsx title="output"
⟦1:<div className="yCard" />⟧
<div ⟦2:className="yCard user"⟧ />
<div ⟦3:className={__yak_mergeClassNames("yCard", on && "on")}⟧ />
```
...
```tsx title="input"
<⟦1:Card⟧ ⟦2:style={{ margin: 1 }} onClick={f} data-x="1"⟧ />
```
...
```tsx title="output"
<⟦1:div⟧ ⟦2:style={{ margin: 1 }} onClick={f} data-x="1"⟧ ⟦1:className="yCard"⟧ />
```
...
```tsx title="output"
const Page = () => <⟦1:Card⟧ className="⟦2:yFancy⟧">hi</⟦1:Card⟧>;
```
...
```tsx title="input"
const ⟦1:Card⟧ = ⟦3:styled.div⟧`
  color: green;
`;

const ⟦2:Fancy⟧ = styled(Card)`
  padding: 4px;
`;
...
```tsx title="output"
const Page = () => <⟦3:div⟧ className="⟦1:yCard⟧ ⟦2:yFancy⟧">hi</⟦3:div⟧>;
```
...
export const ⟦2:Fancy⟧ = styled(Card)`
  padding: 4px;
`;
```
...
```tsx title="output"
export const Fancy = /*#__PURE__*/ __yak.⟦3:__yak_div⟧(
  "⟦1:yCard⟧ ⟦2:yFancy⟧",
);
```
...
```tsx title="input"
const ⟦1:Toggle⟧ = styled.button<{ $on?: boolean }>`
  ${⟦3:({ $on }) =>
    $on &&
    css`
      color: red;
...
```tsx title="output"
const ⟦1:Toggle⟧ = __yak.__yak_button(
  "yToggle",
  ⟦3:({ $on }) => $on && css("yToggle-on")⟧,
);
const ⟦2:Of⟧ = styled(Toggle)("yOf");

const Page = () => <⟦1:Toggle⟧ className="⟦2:yOf⟧">hi</⟦1:Toggle⟧>;
```
...
```tsx title="output"
<div className={"yBox" + (⟦1:true⟧ ? ⟦3:" yBox-on"⟧ : "")} />
<div className={"yBox" + (⟦2:open⟧ ? ⟦3:" yBox-on"⟧ : "")} />
```
...
```tsx title="output"
<div
  className={((⟦2:$on⟧) => "yBox" + ($on ? " yBox-on" : ""))(
    ⟦1:isOpen(item)⟧,
  )}
/>
```
...
<Button ⟦3:disabled⟧={⟦1:busy()⟧}>Save</Button>;
```
...
```tsx title="output"
{
  ((disabled) => (
    <button
      ⟦3:disabled={disabled}⟧
      className={"yButton" + ⟦2:(!disabled ? " yButton-hit" : "")⟧}
    >
      Save
    </button>
  ))(⟦1:busy()⟧);
...
```tsx title="output"
<div className={⟦1:"yBase"⟧ + ⟦2:(a ? " yA" : "")⟧ + ⟦3:(b ? " yB" : "")⟧} />
```
