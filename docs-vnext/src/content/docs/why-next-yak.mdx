...
Most existing CSS-in-JS libraries are either slow or have a complex API. next-yak finds a middle ground between speed and API simplicity and is battle-tested at scale by [Digitec Galaxus](https://www.galaxus.ch), Switzerland's largest e-commerce platform.

The goal of next-yak is to be a CSS-in-JS library with the following properties:

- fast
  - no runtime
  - can be statically extracted
  - can be optimized by the bundler's native CSS tool
...
Optimizations are performed by the bundler's native CSS tool (PostCSS for Next.js with Webpack, Lightning CSS for Turbopack and Vite), allowing you to utilize its full potential.
This also ensures consistency in optimizations across CSS files and CSS-in-JS.

## Our Journey

Our journey with next-yak began at [Digitec Galaxus](https://www.galaxus.ch), in a large Next.js project with more than 150 engineers working on it daily. We extensively used styled-components for flexibility and colocation of styles and code. Despite a few performance issues during server-side rendering, this setup served us well.
...
See related documentation: [Migration from styled-components](/documentation/migration-from-styled-components)
...
<Tabs title="Familiar syntax" groupId="language" items={["typescript", "javascript"]}>
...
<Tabs title="Defining styles" groupId="language" items={["typescript", "javascript"]}>
<Tab value="typescript">
```tsx twoslash
...
<Tabs title="Using props" groupId="language" items={["typescript", "javascript"]}>
<Tab value="typescript">
```tsx twoslash
