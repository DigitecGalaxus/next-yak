...
<Tabs title="Install the addon" items={["npm", "pnpm", "yarn", "bun"]}>
  <Tab value="npm">```bash npm i -D storybook-addon-yak ```</Tab>
  <Tab value="pnpm">```bash pnpm i -D storybook-addon-yak ```</Tab>
  <Tab value="yarn">```bash yarn add -D storybook-addon-yak ```</Tab>
  <Tab value="bun">```bash bun add -D storybook-addon-yak ```</Tab>
</Tabs>
...
<Callout>
  Webpack requires the SWC compiler addon (`@storybook/addon-webpack5-compiler-swc`) for yak-swc
  plugin support.
</Callout>
