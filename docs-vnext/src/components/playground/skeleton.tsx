...
/** line widths of a short code file, so the placeholder reads as code and not as a grid */
const editorLines = [44, 30, 38, 0, 56, 48, 0, 22, 34, 46, 52, 40, 30, 0, 36, 58, 42, 26];
const outputLines = [36, 24, 30, 42, 20, 0, 38, 28, 46, 22];

/**
 * The playground's panels, empty, while the editor and the compiler load. It uses the
 * same layout components as the playground, so the panels appear where they will stay
 * and the page does not jump. It is in the static HTML, so it shows before any script.
 */
export function PlaygroundSkeleton() {
  return (
    <Workspace aria-busy="true">
      ...
      <Card data-ink aria-hidden="true">
        <Header>
...
/* the height of the tab switcher it stands in for, so the header keeps its real height */
