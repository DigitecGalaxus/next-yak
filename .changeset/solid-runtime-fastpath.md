...
The untransformed `styled` export (the mock used without the compiler) turns only string keys into tag factories; `styled.then`, symbols and the function's own properties pass through, so resolving the factory through a promise no longer hangs.
