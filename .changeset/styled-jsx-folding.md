...
Behavioral notes: a folded usage renders the native element directly, so `child.type === Icon` checks don't match it and toggling between a folded and non-folded usage of the same component remounts instead of updating. Foreign `$props` on fully static folded usages are forwarded instead of stripped. `$`-prop expressions on folded usages of dynamic components must be pure: one read by several style expressions is inlined into each and runs once per expression (multiple runs are what StrictMode already does in dev), and one that no style expression reads is dropped without running at all (unobservable as long as it is pure).
