import { styled } from "next-yak/internal";
import { lastChildMixin } from './mixin';
import * as __yak from "next-yak/internal";
import "./index.yak.module.css!=!./index?./index.yak.module.css";
var ListItem = /*YAK EXPORTED STYLED:ListItem:index_ListItem_PZqRtI*//*YAK Extracted CSS:
:global(.index_ListItem_PZqRtI) {
  margin-bottom: 10px;
  &:hover {
    --yak-css-import: url("./mixin:lastChildMixin",mixin);
  }
}
*/ /*#__PURE__*/ Object.assign(/*#__PURE__*/ __yak.__yak_li("index_ListItem_PZqRtI"), {
    "displayName": "ListItem"
});
export default /*YAK EXPORTED STYLED:default:index_ListItem_PZqRtI*//*YAK Extracted CSS:
:global(.index_ListItem_PZqRtI) {
  margin-bottom: 10px;
  &:hover {
    --yak-css-import: url("./mixin:lastChildMixin",mixin);
  }
}
*/ ListItem;